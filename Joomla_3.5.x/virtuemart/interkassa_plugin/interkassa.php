...
class plgVmPaymentInterkassa extends vmPSPlugin
{
  ...
  function plgVmConfirmedOrder($cart,$order)
  {
    ...
    unset($params["secret"]);
		...
		$html = '<form action='.$action_url.' method="POST" name="vm_interkassa_form" id="ikform">
              <input type="hidden" name="ik_am" value="'.$amount.'">
    					<input type="hidden" name="ik_pm_no" value="'.$virtuemart_order_id.'">
    					<input type="hidden" name="ik_desc" value="'.$params['ik_desc'].'">
    					<input type="hidden" name="ik_cur" value="'.$currency.'">
    					<input type="hidden" name="ik_co_id" value="'.$method->merchant_id.'">
              <input type="hidden" name="ik_suc_u" value="'.$success_url.'">
              <input type="hidden" name="ik_fal_u" value="'.$fail_url.'">
              <input type="hidden" name="ik_pnd_u" value="'.$success_url.'">
              <input type="hidden" name="ik_ia_u" value="'.$interaction_url.'">
    					<input type="hidden" name="ik_sign" value="'.$signature.'">
    					<input type="hidden" name="ik_exp" value="'.$dateexp.'">
            </form>
    ...
    if($method->api_status == 1){
      ...
      $ik_dir = JURI::base()."plugins/vmpayment/interkassa/";
      $img_path = $ik_dir."assets/paysystems/";
      $payment_systems = $this->getIkPaymentSystems($method->merchant_id, $method->api_id, $method->api_key);
      $document->addStyleSheet($ik_dir.'assets/ik.css');
      $document->addScript($ik_dir.'assets/ik.js');
      //$html .=
      require 'assets/api.tpl.php';
    }
  ...
  function plgVmOnSelfCallFE ($type, $name, &$render){
    ...
    $sign = $this->IkSignFormation($_POST,$method->secret_key);
    $render->sign = $sign;
    if (isset($_POST['ik_act']) && $_POST['ik_act'] == 'process'){
      $_POST['ik_sign'] = $sign;
      $render = $this->getAnswerFromAPI($_POST);
    }
    //$params = array();
    //parse_str($_POST['form'], $params);
    //$render['sign'] = $this->IkSignFormation($params,$this->secret_key);
    /*
      if (isset($_POST['ik_act']) && $_POST['ik_act'] == 'process'){
        $params['ik_sign'] = $render['sign'];
        $render = $this->getAnswerFromAPI($params);
      }
*/
    //var_dump($render);
    ...
    function wrlog($content){
 	...
 	function checkIP(){
	    ...
	    if(ip2long($_SERVER['REMOTE_ADDR'])<ip2long($ip_stack['ip_begin']) || ip2long($_SERVER['REMOTE_ADDR'])>ip2long($ip_stack['ip_end'])){
	        ...
	        die(JText::_('VMPAYMENT_INTERKASSA_ERR_VOR'));
	    }
	    return true;
    }
}
